...
| IMPR-02-025 | Boundaries | The agent MUST NOT infer approval or approve an Improvement on its own (this is `COR-01-090` applied to Improvement artifacts specifically). A general instruction to improve the harness is not approval of any specific `IMPROVEMENT-NNN`; approval is valid only after the user has had the chance to see that artifact's own proposed change. The agent MUST perform the `proposed` -> `approved` transition, including `approval.approved_by`/`approved_at`, when the user explicitly instructs it to do so, and MUST NOT apply an Improvement's target-file changes before that specific approval exists. |
