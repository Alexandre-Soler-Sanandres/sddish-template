...
| LD-02-025 | Routing | SHOULD prefer candidate Use Cases or Ideas before direct candidate Specs when a finding primarily shapes behavior, actor workflow, or future direction. A direct candidate Spec SHOULD be named only when the evidence already defines a concrete standalone technical contract, proof surface, or implementation-shaping constraint that is not merely downstream of an expected Use Case or Idea. |
