...
| LDC-02-035 | Cross-System-Synthesis | SUMMARY.md's discovery_state MUST be cross-system-active while synthesis is in progress, and MAY move to cross-system-complete only when every planned cross-system slice is done, not-needed, or explicitly deferred, and the Restart Pointer names the next non-discovery phase. This is the SUMMARY.md-scoped counterpart to DISCOVERING-LEGACY.md's Discovery Lifecycle table, which scopes discovery_state to app source maps only. |
| LDC-02-040 | Cross-System-Synthesis | MUST complete reference enrichment for all stable cross-system findings, per DISCOVERING-LEGACY.md's Reference Enrichment table, before SUMMARY.md's discovery_state moves to cross-system-complete (LDC-02-035). |
...
| LDC-05-030 | Slice | Completed Slice Notes in `SUMMARY.md` MUST hold one entry per completed cross-system slice, recording the closeout outputs required by `LDC-06-010` through `LDC-06-055`. |
...
| LDC-07-010 | Blockwise | Blocks are a planning convenience for related cross-system slices with overlapping evidence, not a replacement for slices — MUST mark each included slice individually in SUMMARY.md's Working Discovery Plan status table. |
| LDC-07-020 | Blockwise | MUST use a block only when the grouped slices share a focused evidence set. |
| LDC-07-030 | Blockwise | MUST keep blocks within the active cross-system synthesis scope — MUST NOT span apps or boundaries outside that scope. |
| LDC-07-040 | Blockwise | MUST write one completed block note in SUMMARY.md's Completed Slice Notes listing included slices, evidence paths, stable findings, and unresolved decisions. |
| LDC-07-050 | Blockwise | MUST set the next unfinished slice or coherent block to next after a block completes. |
