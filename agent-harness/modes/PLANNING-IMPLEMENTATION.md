...
| IPL-04-030 | Before requesting a Plan's promotion to `ready`, Planning-Implementation MUST invoke Validation with the profile required by `VAL-00-020-v1` and record the resulting durable report in the Plan when that profile is `chain-preflight`. |
| IPL-04-031 | Planning-Implementation MUST NOT treat local artifact readiness alone as sufficient when `VAL-00-030-v1` requires `chain-preflight` for the selected chain. |
...
| IPL-08-022 | If the cascade lands at Plan-tier, MUST create the Implementation Plan, including a `## Risk-Tier Classification` section (`RSK-05-010-v1`) and setting `entrypoint_type: none` since no Task/Spec/Use Case exists. |
