...
| IMPL-026 | Execution | When a Plan's status changes into or out of `in-progress` (including to `done`), update its row in the `harness-data/CATALOG.md` Active Implementation Plans table in the same pass (COR-057); remove the row once the Plan is no longer `approved` or `in-progress`. |
