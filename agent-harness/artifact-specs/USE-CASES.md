...
| UCS-07-010 | ADR-Check | Missed-ADR recheck. Before setting a Use Case status to `ready-for-spec`, MUST re-run the relevance judgment against the *current* accepted-ADR list: every `fleet-wide` ADR MUST be present in `related`; every `scoped` ADR MUST be re-judged, added if newly relevant, or explicitly ruled out. |
