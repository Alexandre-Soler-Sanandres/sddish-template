...
| IDA-04-010 | Idea-vs-Question | An item is an Idea only if it has a candidate direction — test: "we might build/do X." An open fork with no proposed solution — test: "we haven't decided whether/how to X" — is a Question (`QUESTIONS.md`) instead. A Question resolving toward "yes, pursue this" is what spins off a new Idea, citing the Question's ID as `source`; it never skips straight to becoming one before that resolution happens. |
