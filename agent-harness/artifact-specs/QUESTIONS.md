...
| QST-10-010 | Impact | `Impact` MUST be `active` (currently degrading a running system — worth prioritizing now) or `none` (no current operational impact — a design fork, a someday improvement, or a decision with no live consequence yet); this is a distinct axis from `QST-01-010`'s blast-radius `Classification` and MUST NOT be conflated with it. Defaults to `none` at creation; MUST be set to `active` explicitly, not left to default silently, whenever the raising context already establishes that a presently-broken, in-production capability is involved (mirrors `QST-06-020`'s explicitness standard, applied to this axis). |
