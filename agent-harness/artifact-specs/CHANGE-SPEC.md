...
Frontmatter: `id`, `type: change-spec`, `status`, `lane: standard`, `source_ids`, `related_adrs`, `risk`,
...
All lifecycle states use the same stable file. One explicit current-content approval
...
| CHG-01-040 | MUST stale approval and return to `draft` when `LAN-01-030` material inputs change. |
| CHG-01-041 | MUST use only `draft`, `ready`, `in-progress`, `blocked`, `done`, `archived`, and `rejected`; every transition updates the stable file in place. |
