...
| DEC-11-010 | Resolution-Sync | When an ADR settles a Questions-registry entry and the ADR is set to `accepted`, move that entry's row per `QUESTIONS.md`'s `QST-07-010` in the same pass, citing this ADR's ID. |
