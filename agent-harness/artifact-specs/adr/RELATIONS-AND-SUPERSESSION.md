...
| DEC-07-010 | Relationship-Fields | When a Use Case or Spec is created that depends on an accepted ADR, MUST add the ADR's ID to that artifact's `related` field and add the new artifact's ID to the ADR's own `derived_use_cases`/`derived_specs` field in the same pass. The Task-side equivalent of this obligation lives in `agent-harness/artifact-specs/TASKS.md`'s `TSK-05-010` (Task `related` ↔ ADR `derived_tasks`), not here. |
