...
| IDA-06-010 | Lifecycle | When `/create-use-case`, or (skip-path) `/create-spec`/`/create-tasks`, or a direct Planning-Implementation entry successfully creates the next artifact from this Idea, MUST close this Idea per `IDA-06-030`. |
...
| IDA-06-030 | Lifecycle | Closing an Idea means, as one action: set `status` to `landed`, populate `next` with the citing artifact's ID, and move it to `archive/` per `IDEA.md`'s `## Output / Location`. |
