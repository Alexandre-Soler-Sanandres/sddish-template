...
| UCS-03-010 | Readiness-Gate | Before this Use Case's promotion (`STT-01-030`/`040`), MUST verify the Readiness Checklist in the artifact. |
...
| UCS-05-015 | Carry-Forward | Before advancing to `ready`, MUST verify the registry holds nothing unresolved that should block the advance (see `UCS-03-010`). |
...
| UCS-02-043 | Cascade | Status transitions for this artifact are governed by `systems/STATUS-TRANSITIONS.md`, not defined here. |
...
| UCS-07-010 | ADR-Check | Missed-ADR recheck. Before this Use Case's promotion (`STT-01-030`/`040`), MUST re-run the relevance judgment against the *current* accepted-ADR list. |
...
| UCS-07-020 | ADR-Check | Content-drift recheck. Before this Use Case's promotion (`STT-01-030`/`040`), MUST judge ADR relevance against the Use Case's actual current content, not only its original `area` — drafting can drift the content into territory an ADR bears on that the creation-time load never saw. |
| UCS-07-030 | ADR-Check | Compliance check. Before this Use Case's promotion (`STT-01-030`/`040`), MUST verify every ADR cited in `related` is actually reflected in the Use Case's content — a citation with no matching content is a gate failure. |
