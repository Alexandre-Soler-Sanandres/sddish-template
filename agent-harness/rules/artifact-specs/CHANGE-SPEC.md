...
| CHG-01-041 | Lifecycle | MUST use only `draft`, `ready`, `in-progress`, `blocked`, `done`, `archived`, and `rejected` for a Change Spec; `blocked` remains in `active/`, and `archived` or `rejected` moves to `archive/`. |
