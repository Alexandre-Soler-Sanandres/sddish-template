...
| SPS-05-011 | Carry-Forward | Before setting status to `approved` (see `SPS-02-010`), MUST verify the registry holds nothing unresolved that should block the advance. |
...
| SPS-08-010 | ADR-Check | Missed-ADR recheck. Before setting a Spec status to `approved`, MUST re-run the relevance judgment against the *current* accepted-ADR list. |
