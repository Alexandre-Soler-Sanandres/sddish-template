...
| SPS-01-010 | Sources | MAY create a Spec from a Use Case at any status. |
| SPS-01-012 | Sources | When `RISK-TIER.md`'s Spec-Necessity Matrix (`RSK-03-010`) classifies the request below Spec-tier, `/create-tasks` MAY be entered directly, skipping Spec creation. |
...
| TSK-01-010 | Sources | MAY create Tasks from a Spec at any status. |
| TSK-01-012 | Sources | When `RISK-TIER.md`'s Spec-Necessity Matrix already classified the request below Spec-tier and `TSK-01-030`'s Task-Necessity check requires a Task, it MAY be entered directly. |
...
| TSK-01-032 | Sources | `TSK-01-030`/`TSK-01-031` do not apply when `/create-tasks` is entered normally from a `ready` Spec (`TSK-01-010`'s unconditional path) — that case's Task-necessity was already decided when the Spec was made ready with Tasks in mind. |
