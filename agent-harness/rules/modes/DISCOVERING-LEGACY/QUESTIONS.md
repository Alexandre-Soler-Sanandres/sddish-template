...
| LD-04-040 | Questions | SHOULD default cross-system questions' `Source` to `LF-CROSS-NNNN`, adding app finding IDs in `Notes` only when the extra traceability matters; provenance like slice name or artifact of origin belongs in `Notes`, not the table structure. |
