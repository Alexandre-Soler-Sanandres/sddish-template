...
| RSK-07-010 | Run-Once | The cascade MUST run at most once per request, at the first opportunity it's reached — Partnering's hand-off, Refining's entry, or Planning-Implementation's entry, whichever comes first for that request. The result is carried forward, not recomputed: a skip is recorded via `RSK-05-010`'s `## Risk-Tier Classification` section; a non-skip (full tier required) is evidenced by the artifact's own normal chain (an accepted Use Case behind an approved Spec behind a `ready` Task is itself proof the classification already happened). |
