...
| COR-01-130 | Universal | Status-cascade symmetry across the Use Case → Spec → Task → Plan chain: whenever any artifact in that chain moves off a terminal status (`implemented`, `done`) for a reason other than `IMPLEMENTATION-PLAN.md`'s `IPL-05-080` completion cascade itself (e.g. `USE-CASES.md`'s `UCS-02-040`, `SPECS.md`'s `SPS-07-020`, or a Task/Plan reopened directly), MUST reconsider every other artifact in the chain — in either direction — whose own terminal status was reached via that same `IPL-05-080` completion cascade, and MUST report the reconsideration outcome explicitly ("no change needed" is a valid outcome) rather than leaving an ancestor's or descendant's status claim unexamined by default. `UCS-02-040`/`SPS-07-020`'s own downward resets already satisfy this for the direction they cover; this rule closes the upward direction and any downstream-originated case neither already covers. |
...
| COR-10-020 | Rule-Authoring | When a Review finding traces back to prose-only intent (a shape that tends to recur across a project's Reviews), the resulting Improvement must add the missing rule to the specific file found, and must not treat the instance as fully closed until it does. |
