...
`harness-data/extensions/legacy-discovery/imported/<app-slug>/`
...
See `agent-harness/extensions/legacy-discovery/DISCOVERING-LEGACY.md` for the slice roundtrip, block rules, and content boundaries that
...
see `agent-harness/extensions/legacy-discovery/submodes/IMPORT-HYGIENE.md`'s `LD-07-010`–`050` for the governing rules. A "none found" note is
the expected content when the snapshot is clean, not an empty section.

| Path pattern | Type | Notes |
| --- | --- | --- |

Nested Git metadata check: `find harness-data/extensions/legacy-discovery/imported/<app-slug> -path '*/.git' -type d` — expected: no output.
...
or losing traceability. See `agent-harness/extensions/legacy-discovery/DISCOVERING-LEGACY.md`.

`rewrite-ready` means the app's evidence and cross-system decisions are sufficient to begin rewrite design work.
This is an unblock state, not a claim that every legacy question is closed. Proof obligations and Questions not
required for rewrite-readiness MAY remain, provided they are explicit and tracked in the canonical Questions
registry (referenced from this source map); coherent Use Cases, Specs, and planning MUST still be possible despite
them.

## Reference Enrichment Routing

Route stable findings to the appropriate reference doc before closing the slice — see
`agent-harness/extensions/legacy-discovery/DISCOVERING-LEGACY.md`'s `LD-09-010`/`020` for what qualifies as stable enough to enrich.
...
`agent-harness/artifact-specs/QUESTIONS.md`'s `QST-01-010-v1`: `local`, `cross-artifact`, or `systemic`)
