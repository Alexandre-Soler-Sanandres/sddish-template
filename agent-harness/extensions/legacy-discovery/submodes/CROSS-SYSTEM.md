...
questions, parity evidence, and readiness blockers trace back to those synthesized findings (`LDC-02-030`). Never
...
Cross-system uncertainties use canonical `Q-NNNN` rows in the harness-level Questions registry. Known parity and
proof obligations live on Findings or `REWRITE-READINESS.md`, not in Questions or per-cross-system registries.
...
| Questions registry (`Q-NNNN` rows) | Captures unresolved cross-app or target-product decisions raised by those synthesized findings. |
| Findings and `REWRITE-READINESS.md` | Record known parity gaps and executable proof obligations. |
...
6. Record unresolved cross-app decisions in the Questions registry (`Q-NNNN`).
7. Link parity evidence (`proof obligations`) and readiness blockers back to the finding, question, or proof obligations they depend on.
...
6. Create or update any required Finding or genuine `Q-NNNN` row in the same pass; record known proof obligations
   on the Finding or readiness evidence.
...
11. Record every material unresolved cross-app decision, and create or update a `Q-NNNN` row for each one that is
...
`REWRITE-READINESS.md` by citation, and parity evidence/readiness blockers point back to the finding and proof/
...
- canonical `Q-NNNN` rows for genuine uncertainties and proof obligations on Findings/readiness evidence

## Examples

Two apps both expose a `/health` endpoint with different semantics. The agent records a cross-system finding
citing both apps' app-local findings, raises a `Q-NNNN` row for which semantics the target system should adopt,
and records the parity gap's proof obligation on the Finding and `REWRITE-READINESS.md`.
...
| LDC-01-020 | MUST NOT create `CONTRACTS.md`, Findings, Question rows, or `REWRITE-READINESS.md` speculatively; create them only for evidence or genuine uncertainty and cite app-scoped sources. |
...
| LDC-01-040 | MUST keep known proof/parity obligations on the owning Finding or `REWRITE-READINESS.md` without creating a proof-ID family in Questions. |
| LDC-01-050 | MUST continue new discovery in the scoped structure under `harness-data/extensions/legacy-discovery/`. |
| LDC-01-060 | `REWRITE-READINESS.md` SHOULD reference those existing proof obligations directly instead of inventing a second blocker-ID namespace. |
...
| LDC-02-030 | MUST NOT flatten app-local evidence into cross-system narrative summaries when an app finding or source-map citation is enough. Cross-system synthesis is a second layer over app-local discovery: app-scoped artifacts keep local evidence close to the source app; cross-system findings synthesize boundary claims from multiple app artifacts; cross-system questions, parity evidence, and readiness blockers trace back to those synthesized findings. |
...
| LDC-04-030 | MUST capture a three-way mismatch in a cross-system finding first, route any genuine decision to `Q-NNNN`, and keep the known proof need and planning impact on the Finding or `REWRITE-READINESS.md`. |
...
| LDC-06-040 | MUST complete a fresh-context verification pass — a separate agent invocation receiving only the slice's recorded evidence, findings, Questions/parity evidence, and checklist outputs — before marking a cross-system slice `done`. |
